import { initDB, loadPhotosBySession } from './client/storage.js';
import {
  getActiveSession,
  clearActiveSession,
  getUserId,
} from './client/session.js';
import { trackColorSelected } from './client/analytics.js';
import {
  initCamera,
  startColorSession,
  stopCamera,
...
function renderColor(colorCode) {
  const chip = qs('#color-chip');
  const codeEl = qs('#color-code');
  if (!colorCode) {
...
async function fetchNewColor() {
  ...
  const colorCode = data.colorCode || data.color || '#FFAA33';
  return colorCode;
}

async function uploadCollageToServer(session, blob) {
  try {
    const userId = getUserId();
...
async function handleGenerateCollage() {
  ...
  const canvas = qs('#collage-canvas');
...
function setupEventListeners() {
  qs('#btn-new-color').addEventListener('click', () => {
    handleNewColorClick();
  });

  qs('#btn-start-camera').addEventListener('click', () => {
    handleStartCamera();
  });

  qs('#btn-stop-camera').addEventListener('click', () => {
    handleStopCamera();
  });
  ...
  qs('#btn-generate-collage').addEventListener('click', () => {
    handleGenerateCollage();
  });

  qs('#btn-download-collage').addEventListener('click', () => {
